...
export async function fetchGitHubData(
  ...
  const [user, events1, events2, events3, orgs] = await Promise.all([
  ...
  for (const repo of deepDiveRepos) {
    ...
    const detail = await fetchRepoDetail(owner, repo, username, pat);
    repoDetails.push(detail);
  }

  // Phase 4: Cross-repo contributions
  log('Fetching cross-repo contributions...');
  const crossRepoActivity = await fetchCrossRepoActivity(username, pat);
